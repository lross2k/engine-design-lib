...
float br_combustion()
{
    return const_a_br * pow(press_Pa * fact_conv_br, const_n_br) * fact_conv_i_to_m;
}
...
{
    return PI*Lg*cant_granos*(pow(re, 2)-pow(ri, 2));
}
...
{
    return impulso_esp(list)/(masa_combustible()*g_terr);
}
